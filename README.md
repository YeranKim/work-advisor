...
python scripts/ask.py "사원증 재발급 절차를 알려줘"
# → 맞는 사례 없음 → IT 서비스데스크 (내선 1234) 안내 (발표 자료의 3턴 예시)
```

'변별력 있는 키워드'는 전체 사례의 30% 이하에만 등장하는 토큰이다. '절차'·'신청'처럼 거의 모든 사례에 있는 단어만
겹치는 질의는 BM25 점수가 0 이 아니어도 키워드 미매칭으로 본다.
